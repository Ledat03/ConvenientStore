...
const UpdateImport = ({ isActive, close, handleReload, Import }) => {
  ...
  useEffect(() => {
    if (isActive.updateImport) {
      fetchProducts();
      handleUser();
      const existItem = Import.inventoryImportDetails.map((item) => ({
        product: item.product,
        variant: item.variant,
        quantity: item.quantity,
        cost_price: item.cost_price,
      }));
      setSelectedItems(existItem);
    }
    if (!_.isEmpty(Import)) {
      setFormImport({ importCode: Import.importCode, importNote: Import.importNote });
    }
  }, [isActive.updateImport]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error("Có lỗi xảy ra khi cập nhật đơn nhập hàng");
    } finally {
      setIsLoading(false);
    }
  };
              ...
              <Row className="mb-3">
                <Col md={4}>
                  <Form.Group>
                    ...
                    <Form.Control defaultValue={Import?.importCode} onChange={(e) => setFormImport({ ...formImport, importCode: e.target.value })} placeholder="Ví dụ: NK20240630-01" />
                  </Form.Group>
                </Col>
                <Col md={8}>
                  <Form.Group>
                    <Form.Label>Ghi chú</Form.Label>
                    <Form.Control defaultValue={Import?.importNote} onChange={(e) => setFormImport({ ...formImport, importNote: e.target.value })} placeholder="Ghi chú thêm cho phiếu nhập (nếu có)" />
                  </Form.Group>
                </Col>
              </Row>
